...
rc_validate() {
  ...
  # o2     O2 geometry + O2's own constants. Reproduces the original module
  #        exactly.
  # cache  no O2 at all: cache geometry, and the same constants transcribed
  #        into YO2Compat.h as float, so the arithmetic is bit-identical.
...
rc_patch_geom() {   # file
  ...
  awk -v want="$GEOM_BACKEND" -v mark="$RC_GEOM_MARK" '
    ...
    # lines let a define accumulate and, worse, survive a switch back to o2 --
    # which silently ran the reference mode with the wrong constants.
    # YO2_LOCAL_CONSTANTS is no longer written; it stays in the drop pattern so a
    # job composed by an older runconf.sh is cleaned up rather than inherited.
    $0 == mark { drop = 1; next }
...
rc_compose() {
  ...
  case "$GEOM_BACKEND" in
    ...
    *)
      ...
      _m "                     constants transcribed from O2 into YO2Compat.h as float" ;;
  esac
