...
struct YResidualMonitor {
   ...
   void registerCurvature(double X, double Y, double R){
      curvX = X;
      curvY = Y;
      curvR = R;
   }
   ...
   double chi2ls1[nLAYER+1],chi2ls2[nLAYER+1];   
   double chi2tot;   
...
// Detector 0, Detector(IB/OB1/OB2) 1, HalfStave 2, ChipZ 3, Layer 4, HalfBarrel 5 Stave 6 SubStave 7 // sensor -1 // fix -2
//int kDetectorUnit[7] = {-2, -2, -2, 3, 3, 3, 3};
int kDetectorUnit[7] = {-1, -1, -1, -1, -1, -1, -1};
//int kDetectorUnit[7] = {-1, -1, -1, -1, -1, -1, -1};
struct YDetectorUnit {

   YDetectorUnit() = default;
   YDetectorUnit(short lev, short idx) :
   level(lev),
   index(idx)
   {
      fSCNetwork = new YSensorCorrection(lev, idx);
   };
   ...
   short level; // Detector 0, Detector(IB/OB1/OB2) 1, HalfStave 2, ChipZ 3, Layer 4, HalfBarrel 5 Stave 6 SubStave 7 // sensor -1 // fix -2
...
class YMultiLayerPerceptron : public TMultiLayerPerceptron {
 ...
 public:
   ...
   void GetProjectionPoints(const TVector3 vecCircle_center, const double RecRadius, const TVector3* vecSensorNorm, const TVector3* vecXc_meas, TVector3* vecXc_proj, TVector3* vecXc_norm,
 ...
 protected:
   ...
   void TrackVertexQualityEstimator(double* fparXY, double* fparZR, double* fbeta, double* BeamPos, double* BeamMom,
   ...
   double UpdateVertexByAlignment(bool saveHistory = false);   
   bool TrackerFit(double* input, TVector3* fXFit, double* fparXY, double* fparZR, double* fbeta, std::vector<bool> hitUpdate, double tpc_R, double tpc_Eta);
   void getLineParams(double& x0, double& x1, double& x2,
   ...
   bool VertexerFit(TVector3 parP_j, TVector3 parQ_j, 
   ...
   bool VertexerCheck(TVector3 parP_j, TVector3 parQ_j, 
                      TVector3 parP_k, TVector3 parQ_k,
                      double* BeamMom_j, double* BeamMom_k,
                      TMatrixD& Sum_matInvC, TMatrixD& Sum_matInvCV0,
 ...
 private:
...
#ifdef TRKVTXMONITOR
   TTree* fTrkVtxerMonitor;
   YResidualMonitor* b_trkvtxmonitor;
   bool b_trkvtx_used;
#endif
   //Layer Unit   
   ...
   TF1* fwmajor_csa_L0 = new TF1("fwmajor_csa_L0","[0]", 0, 1);
   TF1* fwminor_csa_L0 = new TF1("fwminor_csa_L0","[0]+[1]*std::pow(x,1)+[2]*std::pow(x,20)", 0, 1);
   TF1* fwz_csa_L0 = new TF1("fwz_csa_L0","[0]+[1]*std::pow(x,1)+[2]*std::pow(x,20)", 0, 1);

   TF1* fwmajor_csa_V = new TF1("fwmajor_csa_V","[0]", 0, 1);
   TF1* fwminor_csa_V = new TF1("fwminor_csa_V","[0]+[1]*std::pow(x,1)+[2]*std::pow(x,30)", 0, 1);
   TF1* fwz_csa_V = new TF1("fwz_csa_V","[0]+[1]*std::pow(x,1)+[2]*std::pow(x,30)", 0, 1);
