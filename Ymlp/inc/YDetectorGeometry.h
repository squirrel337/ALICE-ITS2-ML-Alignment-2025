...
#include <vector>
#ifndef YGEOM_USE_O2
// Pulled in by the O2 headers below when O2 is there; needed explicitly when it
// is not, since TVector3 is this class's return type throughout.
#include "TVector3.h"
#include "TMath.h"
#endif
...
#else
   // Local-to-global transform of a chip as the twelve components of its 3x4
   // affine matrix. Stands in for GetGeom()->getMatrixL2G(id).GetComponents(...)
   // at the two YMultiLayerPerceptron.cxx call sites, so no O2 type appears in
   // the interface. Unused with O2, where those call sites keep the original line.
