...
void check_vertex_pT_plots_trkvtxer_color(int color=-1, int markerstyle=21, double scale = 1.0){
   new TCanvas();
   const int nbinsx = 14;
   const double binsx[] = {0, 0.3, 0.5, 0.75, 1.0, 1.25, 1.50, 2.0, 2.5, 3.0, 3.5, 4.0, 5.0, 6.0, 10.0};
   TH2D* h_event_Vx_pT = new TH2D("h_event_Vx_pT","Vx(cm) vs p_{T}(GeV/c);p_{T}(GeV/c);x(cm)", nbinsx, binsx, 2000, -0.1, 0.1);
   TH2D* h_event_Vy_pT = new TH2D("h_event_Vy_pT","Vy(cm) vs p_{T}(GeV/c);p_{T}(GeV/c);y(cm)", nbinsx, binsx, 2000, -0.1, 0.1);
   TH2D* h_event_Vz_pT = new TH2D("h_event_Vz_pT","Vz(cm) vs p_{T}(GeV/c);p_{T}(GeV/c);z(cm)", nbinsx, binsx, 1500, -15, 15);

   TrkVtxer->Draw("vtxevtX:pT>>h_event_Vx_pT");
   TrkVtxer->Draw("vtxevtY:pT>>h_event_Vy_pT");
   TrkVtxer->Draw("vtxevtZ:pT>>h_event_Vz_pT");

   TH2D* hdcaY_pT   = new TH2D("hdcaY_pT",
                                "dcaY vs p_{T}(GeV/c);p_{T}(GeV/c);dcaY(cm #upoint GeV/c)",
   ...
   TH2D* hdcaZ_pT   = new TH2D("hdcaZ_pT",
                                "dcaZ vs p_{T}(GeV/c);p_{T}(GeV/c);dcaZ(cm #upoint GeV/c)", 
                                nbinsx, binsx, 500, -0.05*cm_to_mu, 0.05*cm_to_mu);
   TH2D* hdcaYZ_pT  = new TH2D("hdcaYZ_pT",
                                "#sqrt{dcaY^{2}+dcaZ^{2}} vs p_{T}(GeV/c);p_{T}(GeV/c);#sqrt{dcaY^{2}+dcaZ^{2}}(cm #upoint GeV/c)", 
   ...
   TrkVtxer->Draw(Form("fip[0]*%g:pT>>hdcaY_pT",cm_to_mu),"");
   TrkVtxer->Draw(Form("fip[0]*%g:pT>>pf_width_dcaY",cm_to_mu),"");
   TrkVtxer->Draw(Form("fip[0]*%g:pT>>pf_widthE_dcaY",cm_to_mu),"");

   TrkVtxer->Draw(Form("fip[1]*%g:pT>>hdcaZ_pT",cm_to_mu),"");
   TrkVtxer->Draw(Form("fip[1]*%g:pT>>pf_width_dcaZ",cm_to_mu),"");
   TrkVtxer->Draw(Form("fip[1]*%g:pT>>pf_widthE_dcaZ",cm_to_mu),"");

   TrkVtxer->Draw(Form("TMath::Sqrt(fip[0]*fip[0]+fip[1]*fip[1])*%g:pT>>hdcaYZ_pT",cm_to_mu),"");

   //fitter
   TH1D* h_mean_dcaY = new TH1D("h_mean_dcaY","(mean) dcaY vs p_{T};p_{T}(GeV/c);dcaY(#mum)",nbinsx,binsx);
   TH1D* h_width_dcaY = new TH1D("h_width_dcaY","(width) dcaY vs p_{T};p_{T}(GeV/c);#sigma(#mum)",nbinsx,binsx);
   for(int ibin = 0; ibin < nbinsx; ibin++){
      ...
      auto proj_hdcaY_pT = (TH1D*) hdcaY_pT->ProjectionY(Form("proj_hdcaY_pT_%d",ibin),1+ibin,1+ibin);

      double minX = proj_hdcaY_pT->GetBinCenter(proj_hdcaY_pT->FindFirstBinAbove(0.5*proj_hdcaY_pT->GetMaximum()));
      double maxX = proj_hdcaY_pT->GetBinCenter(proj_hdcaY_pT->FindLastBinAbove(0.5*proj_hdcaY_pT->GetMaximum()));
      double rangeX = 5*0.5*(std::abs(minX) + std::abs(maxX));
      proj_hdcaY_pT->Fit(fgaus_dcaY,"rQ","",-rangeX,+rangeX);
      proj_hdcaY_pT->Fit(fgaus_dcaY,"rQ","",-rangeX,+rangeX);
      ...
      proj_hdcaY_pT->Fit(fgaus_dcaY,"rQ","", fgaus_dcaY->GetParameter(1) - scale*fgaus_dcaY->GetParameter(2), fgaus_dcaY->GetParameter(1) + scale*fgaus_dcaY->GetParameter(2));
   ...
   TH1D* h_mean_dcaZ = new TH1D("h_mean_dcaZ","(mean) dcaZ vs p_{T};p_{T}(GeV/c);dcaZ(#mum)",nbinsx,binsx);
   TH1D* h_width_dcaZ = new TH1D("h_width_dcaZ","(width) dcaZ vs p_{T};p_{T}(GeV/c);#sigma(#mum)",nbinsx,binsx);
   for(int ibin = 0; ibin < nbinsx; ibin++){
      ...
      auto proj_hdcaZ_pT = (TH1D*) hdcaZ_pT->ProjectionY(Form("proj_hdcaZ_pT_%d",ibin),1+ibin,1+ibin);

      double minX = proj_hdcaZ_pT->GetBinCenter(proj_hdcaZ_pT->FindFirstBinAbove(0.5*proj_hdcaZ_pT->GetMaximum()));
      double maxX = proj_hdcaZ_pT->GetBinCenter(proj_hdcaZ_pT->FindLastBinAbove(0.5*proj_hdcaZ_pT->GetMaximum()));
      double rangeX = 5*0.5*(std::abs(minX) + std::abs(maxX));
      proj_hdcaZ_pT->Fit(fgaus_dcaZ,"rQ","",-rangeX,+rangeX);
      proj_hdcaZ_pT->Fit(fgaus_dcaZ,"rQ","",-rangeX,+rangeX);
      ...
      proj_hdcaZ_pT->Fit(fgaus_dcaZ,"rQ","", fgaus_dcaZ->GetParameter(1) - scale*fgaus_dcaZ->GetParameter(2), fgaus_dcaZ->GetParameter(1) + scale*fgaus_dcaZ->GetParameter(2));
...
/*
   ...
   canvas_vertex->cd(1);
   h_event_Vx_pT->Draw("colz");

   canvas_vertex->cd(2);
   h_event_Vy_pT->Draw("colz");

   canvas_vertex->cd(3);
   h_event_Vz_pT->Draw("colz");
...
*/
   ...
   auto canvas_dca = new TCanvas("canvas_dca_pT","canvas_dca_pT",1600,1000);
