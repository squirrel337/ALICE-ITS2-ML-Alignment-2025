...
void check_vertex_z_plots_trkvtxer_color(int color=-1, int markerstyle=21, double scale = 1.0){
   new TCanvas();
   const int nbinsx = 30;

   TH2D* h_event_Vx_vtxevtZ = new TH2D("h_event_Vx_vtxevtZ","Vx(cm) vs vtxevtZ;vtxevtZ;x(cm)", nbinsx, -15.0, +15.0, 2000,-0.1,0.1);
   TH2D* h_event_Vy_vtxevtZ = new TH2D("h_event_Vy_vtxevtZ","Vy(cm) vs vtxevtZ;vtxevtZ;y(cm)", nbinsx, -15.0, +15.0, 2000,-0.1,0.1);
   TH2D* h_event_Vz_vtxevtZ = new TH2D("h_event_Vz_vtxevtZ","Vz(cm) vs vtxevtZ;vtxevtZ;z(cm)", nbinsx, -15.0, +15.0, 1500,-15,15);

   TrkVtxer->Draw("vtxevtX:vtxevtZ>>h_event_Vx_vtxevtZ");
   TrkVtxer->Draw("vtxevtY:vtxevtZ>>h_event_Vy_vtxevtZ");
   TrkVtxer->Draw("vtxevtZ:vtxevtZ>>h_event_Vz_vtxevtZ");

   TH2D* hdcaY_vtxevtZ   = new TH2D("hdcaY_vtxevtZ",
                                "dcaY vs vtxevtZ;vtxevtZ;dcaY(#mum)",
                                nbinsx, -15.0, +15.0, 500, -0.05*cm_to_mu, 0.05*cm_to_mu);
   TProfile* pf_width_dcaY = new TProfile("pf_width_dcaY","pf_width_dcaY",nbinsx,-15.0,15.0,-0.02*cm_to_mu,0.02*cm_to_mu,"s");
   TProfile* pf_widthE_dcaY = new TProfile("pf_widthE_dcaY","pf_widthE_dcaY",nbinsx,-15.0,15.0,-0.02*cm_to_mu,0.02*cm_to_mu,"g");
   TF1* fgaus_dcaY = new TF1("fgaus_dcaY","gaus",-0.01*cm_to_mu,0.01*cm_to_mu);

   TH2D* hdcaZ_vtxevtZ   = new TH2D("hdcaZ_vtxevtZ",
                                "dcaZ vs vtxevtZ;vtxevtZ;dcaZ(#mum)", 
                                nbinsx, -15.0, +15.0, 500, -0.05*cm_to_mu, 0.05*cm_to_mu);
   TH2D* hdcaYZ_vtxevtZ  = new TH2D("hdcaYZ_vtxevtZ",
                                "#sqrt{dcaY^{2}+dcaZ^{2}} vs vtxevtZ;vtxevtZ;#sqrt{dcaY^{2}+dcaZ^{2}}(#mum)", 
                                nbinsx, -15.0, +15.0,1000,0*cm_to_mu,0.1*cm_to_mu);
   TProfile* pf_width_dcaZ = new TProfile("pf_width_dcaZ","pf_width_dcaZ",nbinsx,-15.0,15.0,-0.02*cm_to_mu,0.02*cm_to_mu,"s");
   TProfile* pf_widthE_dcaZ = new TProfile("pf_widthE_dcaZ","pf_widthE_dcaZ",nbinsx,-15.0,15.0,-0.02*cm_to_mu,0.02*cm_to_mu,"g");
   TF1* fgaus_dcaZ = new TF1("fgaus_dcaZ","gaus",-0.01*cm_to_mu,0.01*cm_to_mu);

   TrkVtxer->Draw(Form("fip[0]*%g:vtxevtZ>>hdcaY_vtxevtZ",cm_to_mu),"TMath::Abs(vtxevtZ)<10");
   TrkVtxer->Draw(Form("fip[0]*%g:vtxevtZ>>pf_width_dcaY",cm_to_mu),"TMath::Abs(vtxevtZ)<10");
   TrkVtxer->Draw(Form("fip[0]*%g:vtxevtZ>>pf_widthE_dcaY",cm_to_mu),"TMath::Abs(vtxevtZ)<10");

   TrkVtxer->Draw(Form("fip[1]*%g:vtxevtZ>>hdcaZ_vtxevtZ",cm_to_mu),"TMath::Abs(vtxevtZ)<10");
   TrkVtxer->Draw(Form("fip[1]*%g:vtxevtZ>>pf_width_dcaZ",cm_to_mu),"TMath::Abs(vtxevtZ)<10");
   TrkVtxer->Draw(Form("fip[1]*%g:vtxevtZ>>pf_widthE_dcaZ",cm_to_mu),"TMath::Abs(vtxevtZ)<10");

   TrkVtxer->Draw(Form("TMath::Sqrt(fip[0]*fip[0]+fip[1]*fip[1])*%g:vtxevtZ>>hdcaYZ_vtxevtZ",cm_to_mu),"TMath::Abs(vtxevtZ)<10");

   //fitter
   TH1D* h_mean_dcaY = new TH1D("h_mean_dcaY","(mean) dcaY vs vtx_{Z};vtx_{Z};dcaY(#mum)",nbinsx,-15.0,15.0);
   TH1D* h_width_dcaY = new TH1D("h_width_dcaY","(width) dcaY vs vtx_{Z};vtx_{Z};#sigma(#mum)",nbinsx,-15.0,15.0);
   for(int ibin = 0; ibin < nbinsx; ibin++){
      ...
      auto proj_hdcaY_vtxevtZ = (TH1D*) hdcaY_vtxevtZ->ProjectionY(Form("proj_hdcaY_vtxevtZ_%d",ibin),1+ibin,1+ibin);

      double minX = proj_hdcaY_vtxevtZ->GetBinCenter(proj_hdcaY_vtxevtZ->FindFirstBinAbove(0.5*proj_hdcaY_vtxevtZ->GetMaximum()));
      double maxX = proj_hdcaY_vtxevtZ->GetBinCenter(proj_hdcaY_vtxevtZ->FindLastBinAbove(0.5*proj_hdcaY_vtxevtZ->GetMaximum()));
      double rangeX = 5*0.5*(std::abs(minX) + std::abs(maxX));
      proj_hdcaY_vtxevtZ->Fit(fgaus_dcaY,"rQ","",-rangeX,+rangeX);
      proj_hdcaY_vtxevtZ->Fit(fgaus_dcaY,"rQ","",-rangeX,+rangeX);
      ...
      proj_hdcaY_vtxevtZ->Fit(fgaus_dcaY,"rQ","", fgaus_dcaY->GetParameter(1) - scale*fgaus_dcaY->GetParameter(2), fgaus_dcaY->GetParameter(1) + scale*fgaus_dcaY->GetParameter(2));
   ...
   TH1D* h_mean_dcaZ = new TH1D("h_mean_dcaZ","(mean) dcaZ vs vtx_{Z};vtx_{Z};dcaZ(#mum)",nbinsx,-15.0,15.0);
   TH1D* h_width_dcaZ = new TH1D("h_width_dcaZ","(width) dcaZ vs vtx_{Z};vtx_{Z};#sigma(#mum)",nbinsx,-15.0,15.0);
   for(int ibin = 0; ibin < nbinsx; ibin++){
      ...
      auto proj_hdcaZ_vtxevtZ = (TH1D*) hdcaZ_vtxevtZ->ProjectionY(Form("proj_hdcaZ_vtxevtZ_%d",ibin),1+ibin,1+ibin);

      double minX = proj_hdcaZ_vtxevtZ->GetBinCenter(proj_hdcaZ_vtxevtZ->FindFirstBinAbove(0.5*proj_hdcaZ_vtxevtZ->GetMaximum()));
      double maxX = proj_hdcaZ_vtxevtZ->GetBinCenter(proj_hdcaZ_vtxevtZ->FindLastBinAbove(0.5*proj_hdcaZ_vtxevtZ->GetMaximum()));
      double rangeX = 5*0.5*(std::abs(minX) + std::abs(maxX));
      proj_hdcaZ_vtxevtZ->Fit(fgaus_dcaZ,"rQ","",-rangeX,+rangeX);
      proj_hdcaZ_vtxevtZ->Fit(fgaus_dcaZ,"rQ","",-rangeX,+rangeX);
      ...
      proj_hdcaZ_vtxevtZ->Fit(fgaus_dcaZ,"rQ","", fgaus_dcaZ->GetParameter(1) - scale*fgaus_dcaZ->GetParameter(2), fgaus_dcaZ->GetParameter(1) + scale*fgaus_dcaZ->GetParameter(2));
...
/*
   ...
   canvas_vertex->cd(1);
   h_event_Vx_vtxevtZ->Draw("colz");

   canvas_vertex->cd(2);
   h_event_Vy_vtxevtZ->Draw("colz");

   canvas_vertex->cd(3);
   h_event_Vz_vtxevtZ->Draw("colz");

   canvas_vertex->cd(4);
   hdcaY_vtxevtZ->Draw("colz");

   canvas_vertex->cd(5);
   hdcaZ_vtxevtZ->Draw("colz");

   canvas_vertex->cd(6);
   hdcaYZ_vtxevtZ->Draw("colz");
*/

   auto canvas_dca = new TCanvas("canvas_dca_vtxevtZ","canvas_dca_vtxevtZ",1600,1000);
